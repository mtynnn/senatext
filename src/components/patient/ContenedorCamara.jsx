...
export const ContenedorCamara = () => {
  ...
  return (
    <div className="relative w-full aspect-video bg-slate-100 border-2 border-slate-300 rounded-2xl p-2 flex flex-col items-center justify-center max-h-[300px] sm:max-h-[350px] md:max-h-[380px] min-h-[230px] overflow-hidden group mx-auto">
      ...
      <video
        ref={referenciaVideo}
        className={`absolute inset-0 w-full h-full object-cover rounded-3xl z-0 ${(!camaraActiva || !camaraLista) ? 'opacity-0' : 'opacity-100'}`}
        playsInline
        muted
      />
      <canvas
        ref={referenciaLienzo}
        className={`absolute inset-0 w-full h-full pointer-events-none rounded-3xl z-10 ${(!camaraActiva || !camaraLista) ? 'opacity-0' : 'opacity-100'}`}
      />
